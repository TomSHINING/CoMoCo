...
def interpolate1d_cuda(array, pos):
    ...
    # Check boundary conditionsfile:///D:/xwechat_files/wxid_r3w0rgij2kc222_137c/temp/InputTemp/4e3c85d4-264d-4b5f-8563-fd16d5b4dae7.png
...
def params_3d_proj_matrix(angles, dsd, dsi, tx, ty, det_spacing, det_origin,source_height):

    """
    compute cone-beam projection matrices from parameters for circular trajectory in 3D

    :param angles: projection angles in radians
    :param dsd: source to detector distance
    :param dsi: source to isocenter distance
    :param tx: detector offset x
    :param ty: detector offset y
    :param tz: detector offset z
    :param det_spacing: detector pixel size (assume square pixels)
    :param det_origin: detector origin (usually (image_size - 0.5))
    :return:
    """
    num_angles = len(angles)
    matrices = np.zeros((num_angles, 3, 4))

    # 3D extrinsic: rotate around z-axis (circular trajectory)
    extrinsics = np.zeros((num_angles, 3, 4))
    extrinsics[:, 0, 0] = np.sin(angles)
    extrinsics[:, 0, 1] = -np.cos(angles)
    extrinsics[:, 0, 2] = 0
    extrinsics[:, 0, 3] = tx
    ...
    extrinsics[:, 1, 1] = 0
    extrinsics[:, 1, 2] = 1
    extrinsics[:, 1, 3] = -source_height

    extrinsics[:, 2, 0] = np.cos(angles)
    extrinsics[:, 2, 1] = np.sin(angles)
    extrinsics[:, 2, 2] = 0
    extrinsics[:, 2, 3] = dsi + ty

    # Intrinsic matrix for detector (u, v)
    intrinsics = np.zeros((num_angles, 3, 3))
    intrinsics[:, 0, 0] = dsd /det_spacing  # scaling u
    intrinsics[:, 0, 2] = det_origin[0]  # u offset
    intrinsics[:, 1, 1] = dsd /det_spacing  # scaling v
    intrinsics[:, 1, 2] = det_origin[1]  # v offset
    intrinsics[:, 2, 2] = 1.0  # homogeneous

    # compute projection matrices
    for i in range(num_angles):
        matrices[i] = intrinsics[i] @ extrinsics[i]

    # normalize w.r.t. lower right entry
    matrices = matrices / matrices[:, 2, 3][:, np.newaxis, np.newaxis]
